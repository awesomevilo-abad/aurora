                                            ...
                                            if(isset($Pid)){
                                                ...
                                                <a href="#" onclick="loadCat()" ><h5 style="text-decoration:none;text-align:center;color:#fff;background-color:#dfdfdf" class="mg-title text-bold">Back</h5></a>
                                                    <div class="thumbnail" style="background-color:#34495e;border-radius:10%;height:400;">
                                                            ...
                                                            (SELECT COUNT(countAid) as countAid FROM (SELECT Aid countAid from checklist_grade where Pid ='$Pid' AND status = 'Phase On Going'  GROUP BY Aid ORDER BY Aid DESC) as aidTable) as countAid 
                                                            from area 
                                                            where Pid ='$Pid' 
                                                            ORDER BY Aid DESC 
                                                            LIMIT 1
                                                            ...
                                                            if(($rowgetAuditAreaPerPhase['countAid'] != 0) && ($AuditCutoffFormat == $DatetodayMinus1)){
                                                                ...
                                                                WHERE Pid = '$Pid' AND Aid NOT IN (
                                                                    ...
                                                                    WHERE Pid = '$Pid' AND status = 'Phase On Going' AND Aid IN (SELECT Aid from area where Pid = '$Pid') GROUP BY Aid)
                                                                ");
