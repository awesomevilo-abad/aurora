<?php 
    ...
    $getArea = $conn->prepare("SELECT Aid FROM area WHERE Pid = :Pid ");
    $getArea->execute(array(":Pid"=> $_POST['pid']));
    while($rowgetArea = $getArea->fetch(PDO::FETCH_ASSOC)){

        $countOfAllAreasInPhase[] = $rowgetArea['Aid'];
    }
   ...
   $countAllAreasAudit = sizeof($allareaAid);
   $countAllAreas = sizeof($countOfAllAreasInPhase);
   $countFinalArea = $countAllAreas-1;

    if($countAllAreasAudit == $countFinalArea){
        $cid = $_POST['cid']; //array
    ...
    $gradesheader = $conn->prepare("SELECT * FROM checklist_grade WHERE Pid=:Pid  AND  (Date_Checked >= '$Datetodayminusfive' AND Date_Checked <= '$Datetoday') ORDER BY Aid DESC ");
    $gradesheader->execute(array(":Pid"=> $pid));
