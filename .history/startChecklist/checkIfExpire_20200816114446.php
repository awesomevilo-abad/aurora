...
while($rowgetPid = $getPid->fetch(PDO::FETCH_ASSOC)){
   ...
   echo $Pid = $rowgetPid['Pid'];
    ...
    $getPidStatus = $conn->prepare("SELECT * FROM checklist_grade WHERE Bid = '1001'   AND (Date_Checked >= '2020-08-12' AND Date_Checked <= '2020-08-16') GROUP BY Pid ORDER BY Aid ASC");
