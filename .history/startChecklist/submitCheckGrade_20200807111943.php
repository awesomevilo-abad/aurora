<?php 
    ...
    $gradesheader = $conn->prepare("SELECT * FROM checklist_grade WHERE Pid = :Pid AND Date_Checked = :datetoday GROUP BY Aid DESC ");
    $gradesheader->execute(array(":Pid"=> $_POST['pid'],":datetoday"=> $Datetoday));
    $rowgradesheader = $gradesheader->fetch(PDO::FETCH_ASSOC);
    $areaAid = $rowgradesheader['Aid'];
