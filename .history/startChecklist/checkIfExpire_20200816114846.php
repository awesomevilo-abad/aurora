...
while($rowgetPidStatus = $getPidStatus->fetch(PDO::FETCH_ASSOC)){
    ...
    $getOnGoingPid = $conn->prepare("SELECT * FROM checklist_grade WHERE Pid = '$audittedPid'   AND (Date_Checked >= '$Datetodayminusfive' AND Date_Checked <= '$Datetoday') GROUP BY Pid ORDER BY Aid ASC");
    $getPidStatus->execute(array(":Bid"=> $_POST['id']));
    while($rowgetPidStatus = $getPidStatus->fetch(PDO::FETCH_ASSOC)){
