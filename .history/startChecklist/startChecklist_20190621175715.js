...
function ConfirmationModal(id,pageType) {
	$("#divyear").hide();
	$("#confirmModal").click();
...
$("#formGrade").on('submit', (function (e) {
	...
	$.ajax({
		...
		success: function (data) {
			...
			if (data == "Next Area") {
				$("#btnNextArea").hide();
				window.location.assign('MainChecklist.php?id=' + phaseid + "&week="+week + "&month="+month + "&year="+year);
			} else if (data == "Next Phase") {
				$("#btnNextArea").hide();
				window.location.assign('viewing_gradephase.php?id=' + phaseid + "&week="+week + "&month="+month + "&year="+year);
			} else {
...
function getID(position, id) {
	if($("#week").val() == " "){
		$("week").focus();
		$('#week').css({'border': '2px solid #d2322d'});
		...
		$.ajax({
			...
			success: function (data) {
				if (data == "QA Staff" || data == "QA Supervisor" && pageType == "Audit") {
					location.href = "MainChecklist.php?id=" + getId + "&pageType="+pageType + "&week="+week + "&month="+month + "&year="+year;
...
function viewPhase(id,pageType) {
	$.ajax({
