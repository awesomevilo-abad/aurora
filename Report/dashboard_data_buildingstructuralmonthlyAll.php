<?php
	...
	$conn = $dao->openConnection();
                ...
                FROM(SELECT tid,month,week,pid,id,Name,PName, AVG(protect_stru_grade) as newWeekGrade 
                FROM (SELECT timedatephase.tid,timedatephase.month,timedatephase.week,timedatephase.protect_stru_grade, timedatephase.pid,building.id, building.Name,phase.PName 
                FROM timedatephase 
                left join phase on timedatephase.pid = phase.Pid 
                left join building on phase.Bid = building.id 
                Where timedatephase.year = '".$year."' 
                GROUP BY timedatephase.month,timedatephase.pid,timedatephase.week,timedatephase.Bid 
                ORDER by timedatephase.protect_stru_grade DESC)as newWeekGrade 
