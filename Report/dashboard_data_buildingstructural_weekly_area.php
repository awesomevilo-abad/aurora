<?php
	...
	$conn = $dao->openConnection();
                ...
                $phase = $_POST['phase'];
				$building = [];
				$grades = $conn->prepare("SELECT Accounts.AcName,timedatephase.qastaff,checklist_grade.Pid,checklist_grade.totalstrugrade,area.AName,timedatephase.week,timedatephase.protect_stru_grade, timedatephase.pid,building.id, building.Name,phase.PName
				...
				while($rowGrades = $grades->fetch(PDO::FETCH_ASSOC)){
					...
					array_push($building, [
                        ...
                        'Protech'   => $protech,
                        'type' => 'column',
                        'y' => floatval($rowGrades['totalstrugrade'])
					  ]);
				  
                }

                $areaWeight =array();
				$grades = $conn->prepare("SELECT area.Percentage,Accounts.AcName,timedatephase.qastaff,checklist_grade.Pid,checklist_grade.totalstrugrade,area.AName,timedatephase.week,timedatephase.protect_stru_grade, timedatephase.pid,building.id, building.Name,phase.PName
                FROM timedatephase left join phase on timedatephase.pid = phase.Pid 
                left join building on phase.Bid = building.id 
                left join area on timedatephase.pid = area.Pid
                left join checklist_grade on area.Aid  = checklist_grade.Aid
