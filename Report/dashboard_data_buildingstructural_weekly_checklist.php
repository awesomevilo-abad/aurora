<?php
	...
	$conn = $dao->openConnection();
				...
				$building = [];
				$grades = $conn->prepare("SELECT Accounts.AcName,timedatephase.qastaff,checklist_grade.Pid,checklist_grade.CName,checklist_grade.Str_Grade,checklist_grade.totalstrugrade,area.AName,timedatephase.week,timedatephase.protect_stru_grade, timedatephase.pid,building.id, building.Name,phase.PName
				...
				while($rowGrades = $grades->fetch(PDO::FETCH_ASSOC)){
					...
					array_push($building, [
                        ...
                        'Protech'   => $protech,
                        'type' => 'column',
                        'y' => floatval($rowGrades['Str_Grade']),
