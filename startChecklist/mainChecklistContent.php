    ...
    <div id="checklist" class="col-md-12">
        <form method="POST" action="viewing_grade.php" id="saveImage" enctype="multipart/form-data">
                       ...
                       <?php
                        ...
                        // Show Another area if area is auditted
                        else if($gradesheader->rowCount() > 0){
                            if(trim($selectedaidfromSkippedArea)=="none"){
                                $showArea = $conn->prepare("SELECT * FROM area WHERE Pid = :Pid AND 
                                Aid NOT IN (SELECT Aid FROM checklist_grade WHERE Pid=:Pid  AND (Date_Checked >= :from AND Date_Checked <= :to))
                                ORDER BY Aid ASC");
                                $showArea->execute(array(":Aid" => $rowgradesheader['Aid'],":Pid"=> $id,":from"=> $Datetodayminusfive,":to"=> $Datetoday));
                                $rowareaheader = $showArea->fetch(PDO::FETCH_ASSOC);
                            }else{
                                $showArea = $conn->prepare("SELECT * FROM area WHERE Aid = :Aid ORDER BY Aid");
                        ...
                        // Show First Area if it is a First Audit in Phase within the day
                        else{
