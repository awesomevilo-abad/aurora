  ...
  $Datetoday = $crudcontroller->getDate();
        ...
        if($_FILES['addmoreImg']['name'][0] != ""){
            ...
            $aid = $_POST['Aid'];
            $cid = $_POST['ChecklistId'];
            $image =  $_FILES['addmoreImg']['name'];
            $eid = "0000";
            $Datetoday = $_POST['Date'];
        
            $countimage = sizeof($image);
            // echo "<div style='float:right;border:1px solid #e63f1b;background-color:#ff7152;border-radius:10px;width:50px;color:#ffffff;text-align:center;cursor:pointer;' onclick='DeleteImage(".$_POST['ChecklistId'].");'>RESET</div>";
            for($i=0;$i<$countimage;$i++){
              ...
              $stmtshowImage->bindParam(":cid", $cid);
